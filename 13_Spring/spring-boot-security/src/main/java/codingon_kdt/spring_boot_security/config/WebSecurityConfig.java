...
@EnableWebSecurity      // 웹 Security 기능을 활성화
@Slf4j
public class WebSecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http.cors(withDefaults())       // cors 기본 설정
                .csrf(CsrfConfigurer::disable)      // csrf 공격 보호 설정
        ...
        // filter 등록: 매 요청마다 CorsFilter 를 실행한 뒤에 jwtAuthenticationFilter 실행
    ...
    public CorsConfigurationSource corsConfigurationSource() {
        ...
        config.setAllowedMethods(Arrays.asList("HEAD", "GET", "POST", "PUT", "DELETE", "PUT", "PATCH"));
